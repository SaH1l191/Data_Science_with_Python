...
from (
	...
	row_number() 
	over(partition by cuisine order by cost*rating desc) as rank
	from restaurants
	) t
	where t.rank < 6 group by cuisine order by revenue desc;

-- 2. What is the of the total revenue is generated by top 1% restaurants
 select sum(cost*rating_count) as revenue from
(
	select *, cost*rating_count as tot_rev, 
	row_number() 
	over(order by cost*rating_count desc) as rank
	from restaurants
) t
where t.rank <= (
	614
...
select (top_revenue/total_revenue)*100  as revenuepercen from q1,q2;
